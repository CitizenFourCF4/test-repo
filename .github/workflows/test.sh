...
curl -d '{"chat_id":"$CHAT_ID", "text":"$MESSAGE", "parse_mode": "$PARSE_MODE", "reply_markup": {"inline_keyboard": [[{"text":"Открыть", "url": "$PR_LINK"}]]} }' -H "Content-Type: application/json" -X POST https://api.telegram.org/bot$BOT_API_KEY/sendMessage
